...
const navItems: NavItem[] = [
  ...
  { icon: <FaCashRegister className="w-5 h-5" />, name: "Punto de Venta", path: "/punto-venta" },
  { icon: <FaShoppingCart className="w-5 h-5" />, name: "Ventas", path: "/ventas" },
  { icon: <FaShoppingBag className="w-5 h-5" />, name: "Compras", path: "/compras" },
  {
...
const AppSidebar: React.FC = () => {
        ...
        <li key={nav.name}>
          {nav.subItems ? (
            ...
            >
              ...
              {(isExpanded || isHovered || isMobileOpen) && <FaChevronDown className={`ml-auto w-4 h-4 transition-transform duration-200 ${openSubmenu?.type === menuType && openSubmenu?.index === index ? "rotate-180 text-brand-500" : ""}`} />}
            </button>
          ) : (
            nav.path && (
              <Link
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col overflow-y-auto duration-300 ease-linear no-scrollbar items-center">
        <nav className="mb-6">
          <div className="flex flex-col gap-4">
            <div>
              <h2 className={`mb-4 text-xs uppercase flex leading-[20px] text-gray-400 ${!isExpanded && !isHovered ? "lg:justify-center" : "justify-start"}`}>
                {isExpanded || isHovered || isMobileOpen ? "Menú Principal" : <FaBars />}
              </h2>
              {renderMenuItems(navItems, "main")}
            </div>
          </div>
        </nav>
        <div>{isExpanded || isHovered || isMobileOpen ? <SidebarWidget /> : null}</div>
      </div>
    </aside>
  );
};
