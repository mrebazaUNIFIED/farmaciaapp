...
export const PuntoVenta = () => {
	...
	setLoading(true)
	try {
		// Función para generar numero_venta (implementa esto o similar)
		const generarNumeroVenta = async () => {
			...
			const ultimoNumero = ultimaVenta?.[0]?.numero_venta || 'V-00000';
			const numero = parseInt(ultimoNumero.split('-')[1]) + 1;
			return `V-${numero.toString().padStart(5, '0')}`;
		};

		// Calcula subtotal, igv y total (asumiendo IGV 18%)
		const calcularSubtotal = () => carrito.reduce((sum, item) => sum + item.subtotal, 0);
		const calcularIgv = () => calcularSubtotal() * 0;
		const calcularTotal = () => calcularSubtotal() + calcularIgv();

		const venta = {
			cliente_id: null,
			usuario_id: user?.id || null, // Corregido a 'usuario_id'
			numero_venta: await generarNumeroVenta(), // Requerido
			subtotal: calcularSubtotal(), // Requerido
			igv: calcularIgv(), // Requerido
			total: calcularTotal(),
			metodo_pago: metodoPago,
			estado: 'COMPLETADA', // Mayúsculas para coincidir con esquema
		}
